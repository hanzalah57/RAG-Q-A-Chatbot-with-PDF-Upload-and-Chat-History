    ...
    with st.spinner("Loading and Splitting PDFs..."):
        for pdf in uploaded_files:
            ...
            with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
                tmp.write(pdf.getvalue())  # Use getvalue() to read uploaded file bytes
                pdf_path = tmp.name
...
#Splits docs into Chunks and Embeddings
...
if not splits:
    st.warning("No text chunks found...Please upload atleast one valid PDF")
    st.stop()
...
qa_prompt  = ChatPromptTemplate.from_messages([
    ("system", "Use the retrieved context to answer."
                "If you don't know, say so. Keep it brief and descriptive.\n\n{context}"),
    MessagesPlaceholder("chat_history"),
    ("human","{input}"),
])
